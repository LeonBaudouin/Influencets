...
export default class PositionOverTime extends AbstractController {
    ...
    update(currentState: SizeState): SizeState {
        ...
        newState.position.x = this.defaultPos.x + this.XFunc(this.time, this.amount, this.offset);
        newState.position.y = this.defaultPos.y + this.YFunc(this.time, this.amount, this.offset);
        this.time++;
        return newState;
    }
